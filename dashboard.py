...
let lastView='';
async function refresh(force=false){const expanded=[...document.querySelectorAll('details.run[open]')];if(expanded.length&&!force)return;const openRuns=new Set(expanded.map(e=>e.dataset.runId));const data=await fetch('/api/runs').then(r=>r.json());document.querySelectorAll('details.run[open]').forEach(e=>openRuns.add(e.dataset.runId));const q=document.querySelector('#filter').value.toLowerCase();const view=JSON.stringify([data,q]);if(view===lastView)return;lastView=view;
...
document.querySelector('#filter').addEventListener('input',()=>refresh(true));refresh();setInterval(()=>refresh(false),3000);
